...
import static org.example.cryptography.cryptography.Algorithms.LOKI97.Operations.IndexingRule.FROM_MOST_TO_LEAST_START_WITH_0;
...
public class EncryptionConversionFeistelFunctionLOKI97 implements EncryptionConversion {
    ...
    private void initSBoxes() {
        ...
        for (int i = 0; i < S1.length; i++) {
            int x = (~i ^ 0x1FFF) & 0x1FFF; // 13 бит
            S1[i] = (byte) (modPow(x, 3, 0x2911) & 0xFF);
        }

        for (int i = 0; i < S2.length; i++) {
            int x = (~i ^ 0x7FF) & 0x7FF; // 11 бит
            S2[i] = (byte) (modPow(x, 3, 0x0AA7) & 0xFF);
        }
